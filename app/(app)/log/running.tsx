...
export default function RunningLog() {
  ...
  function getDistanceM(): number {
    return (parseFloat(distanceKm) || 0) * 1000;
  }
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const avgPace = calcPaceSecPerKm(distM, durSec);
      const estimatedCal = profile?.weight_kg
        ? estimateCalories('running', durSec, profile.weight_kg)
        : undefined;
      ...
      const id = await saveWorkout(db, {
        ...
        duration_sec: durSec,
        distance_m: distM,
        calories: calories ? parseInt(calories) : estimatedCal,
        ...
        humidity_pct: humidity ? parseInt(humidity) : undefined,
        running: {
      ...
      if (profile && checkAndIncrementAIUsage(0.5)) {
        (async () => {
          try {
            ...
            const recentSame = await getWorkoutsBySport(db, 'running', 4);
            if (workout) {
  ...
  return (
    ...
    >
      <ScreenHeader title="🏃 러닝 기록" showBack />
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Field label="시간 (시:분:초) *">
          <View style={styles.timeRow}>
            <TextInput style={[styles.input, styles.timeInput]} value={durationHH} onChangeText={setDurationHH} keyboardType="number-pad" placeholder="0" placeholderTextColor={colors.textMuted} maxLength={2} />
            <Text style={styles.timeSep}>:</Text>
            <TextInput style={[styles.input, styles.timeInput]} value={durationMM} onChangeText={setDurationMM} keyboardType="number-pad" placeholder="25" placeholderTextColor={colors.textMuted} maxLength={2} />
            <Text style={styles.timeSep}>:</Text>
        ...
        <View style={styles.row}>
          <View style={styles.half}>
            <Field label="평균 심박수 (bpm)">
              <TextInput style={styles.input} value={avgHR} onChangeText={setAvgHR} keyboardType="number-pad" placeholder="150" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
          <View style={styles.half}>
            <Field label="최대 심박수 (bpm)">
              <TextInput style={styles.input} value={maxHR} onChangeText={setMaxHR} keyboardType="number-pad" placeholder="175" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
        </View>
        ...
        <View style={styles.row}>
          <View style={styles.half}>
            <Field label="케이던스 (spm)">
              <TextInput style={styles.input} value={cadence} onChangeText={setCadence} keyboardType="number-pad" placeholder="170" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
          <View style={styles.half}>
            <Field label="고도 상승 (m)">
              <TextInput style={styles.input} value={elevation} onChangeText={setElevation} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
        </View>
        ...
        <View style={styles.row}>
          <View style={styles.half}>
            <Field label="기온 (°C)">
              <TextInput style={styles.input} value={tempC} onChangeText={setTempC} keyboardType="decimal-pad" placeholder="20" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
          <View style={styles.half}>
            <Field label="습도 (%)">
              <TextInput style={styles.input} value={humidity} onChangeText={setHumidity} keyboardType="number-pad" placeholder="60" placeholderTextColor={colors.textMuted} />
            </Field>
          </View>
        </View>

        <Field label="칼로리 (자동 계산됨)">
          <TextInput style={styles.input} value={calories} onChangeText={setCalories} keyboardType="number-pad" placeholder={profile?.weight_kg ? `약 ${estimateCalories('running', getDurationSec(), profile.weight_kg)} kcal` : '450'} placeholderTextColor={colors.textMuted} />
        </Field>
...
const styles = StyleSheet.create({
  ...
  chipActive: { borderColor: colors.running, backgroundColor: '#1A0A0A' },
  chipText: { color: colors.textSecondary, fontSize: 13 },
  chipTextActive: { color: colors.running, fontWeight: '600' },
